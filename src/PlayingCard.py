import random #, playing_card_tests
...
def deal_cards(deck, no_of_cards, no_of_hands):
    hands=[]
    all_cards = False
    if no_of_cards == 0:
    ...
    for hand_index in range(0, no_of_hands):
        while len(hands[hand_index]) < no_of_cards and len(deck) > 0:
            dealt_card = deal_a_card(deck)
            hands[hand_index].append(dealt_card)

    if all_cards:
        counter = 0
        while len(deck) > 0:
            hands[counter].append(deal_a_card(deck))
...
# the hand.
def play_a_card(hand, card_to_play):
...
def is_playing_a_card(hand, card_to_play):
    played = False
...
# HQ -> H12
# CK -> C13
def convert_face_to_number(card):
...
def convert_faces_to_numbers(hand):
    for counter in range(0, len(hand)):
        hand[counter] = convert_face_to_number(hand[counter])
...
def convert_number_to_face(card):
    face = card[1:len(card)]
    if face == "13":
        new_face = "K"
    elif face == "12":
...
def convert_numbers_to_faces(hand):
    for counter in range(0, len(hand)):
        hand[counter] = convert_number_to_face(hand[counter])
